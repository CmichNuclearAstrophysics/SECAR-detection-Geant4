...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
    if(source)
    {
        fParticleGun->SetParticlePosition(G4ThreeVector(0,0,-52*mm));

        // Distribution uniform in solid angle
        G4double Theta = 23*(2*G4UniformRand()-1);
        G4double cosTheta = std::cos(Theta);
        //G4double cosTheta = 0.920505 + (1.0 - 0.920505)*G4UniformRand();
    ...
    {  
        ...
        G4double energy, theta, z;
        //G4double energy, theta;
        if(n==1) OpenExternalFile();
        
        input_file >> z >> theta >> energy;
        ...
        else theta_n = CLHEP::RandFlat::shoot(theta, theta+0.002) + 0.000001;


        fParticleGun->SetParticlePosition(G4ThreeVector(0,0,z*mm));
        //fParticleGun->SetParticlePosition(G4ThreeVector(0,0,0));
        
        // Uniform spherical distribution
        // Converting the spherical coordinates (polar and azimuthal angles) to Cartesian coordinates. 
        G4double phi = CLHEP::RandFlat::shoot(0.0,360.); //random angle between 0-360 degrees
        //G4double phi = twopi*G4UniformRand();
        //theta_n = theta_n + G4UniformRand();
        // G4double cosMin = std::cos((theta_n+1)*deg);
        // G4double cosMax = std::cos(theta_n*deg);
        // G4double cosTheta = cosMin + (cosMax - cosMin)*G4UniformRand();
        // G4double sinTheta = std::sqrt(1. - cosTheta*cosTheta);
        //G4double cosTheta = cos(theta_n*deg)+cos((theta_n+1.)*deg)*G4UniformRand(); //incorrect?
        
        G4double vx = sin(theta_n*deg)*cos(phi*deg);
        G4double vy = sin(theta_n*deg)*sin(phi*deg);
        G4double vz = cos(theta_n*deg);

        // G4double vx = sinTheta*std::cos(phi*deg);
        // G4double vy = sinTheta*std::sin(phi*deg);
...
{   
    ...
    if(pn_neutrons)
    input_file.open("../neutronInput/test.txt",std::ios::in);
    //input_file.open("../accurate_kinematic_neutrons.txt",std::ios::in);

    
    if(pn_recoils)
    input_file.open("../recoil_input/test.txt",std::ios::in);
